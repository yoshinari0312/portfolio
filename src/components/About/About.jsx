      ...
      <div className='aboutTimeline'>
        ...
        >
          <AboutTimelineItem
            ...
            outline={
              <AboutOutline>
                {["#柔道部に所属", "#県大会優勝 #全国大会出場"].map((item, index) => (
                  <span key={`middle-${index}`}>{item}</span>
                ))}
              </AboutOutline>
            }
          />
          <AboutTimelineItem
            ...
            outline={
              <AboutOutline>
                {["#卒業式で成績優秀者として表彰"].map((item, index) => (
                  <span key={`highschool-${index}`}>{item}</span>
                ))}
              </AboutOutline>
            }
          />
          <AboutTimelineItem
            ...
            outline={
              <AboutOutline>
                {[
                  <span key="university-0">#<Link url='https://www.ailab.ics.keio.ac.jp/'>今井倫太研究室</Link>に所属し、AIロボティクスについて研究</span>,
                  <span key="university-1">#卒業式で首席として<Link url='http://www.keiokougakukai.org/sub-text/20230323kougakukaisyou.html'>慶應工学会賞</Link>を受賞</span>
                ]}
              </AboutOutline>
            }
          />
          <AboutTimelineItem
            ...
            outline={
              <AboutOutline>
                {["#修士1年生", "#心理学部の教授とAIロボティクスに関する共同研究"].map((item, index) => (
                  <span key={`gradschool-${index}`}>{item}</span>
                ))}
              </AboutOutline>
            }
          />
        ...
        <div className='aboutPicture'>
          <Avatar alt="顔写真" src={`${process.env.PUBLIC_URL}/img/AboutImages/face.jpg`} sx={{ width: 250, height: 250}} />
          <p>小野寺佳成</p>
        </div>
      </div>
    </Layout>
